...
import { assertUserCanAuthenticate } from "./assertUserCanAuthenticate";
...
export const authService = {
  ...
  login: async (res: Response, payload: Partial<IUser>) => {
    ...
    const { accessToken, refreshToken } = createUserAuthTokens(user);
    setAuthCookie(res, {
      accessToken,
      refreshToken,
    });

    await cacheService.set(CACHE_KEYS.USER_BY_EMAIL(normalizedEmail), user, 60);
